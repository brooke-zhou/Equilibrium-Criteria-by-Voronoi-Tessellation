...
for timestep in range(1,4):
    q_by_type=[[1., 0., 0., 0.],
            [2.*timestep, 2.*timestep, 1.*timestep, 5.*timestep],
            [3.*timestep, 2.*timestep, 4.*timestep, 3.*timestep],
            [4.*timestep, 3.*timestep, 8.*timestep, 4.*timestep],
            [5.*timestep, 4.*timestep, 7.*timestep, 4.*timestep],
            [6.*timestep, 2.*timestep, 8.*timestep, 7.*timestep]]
    for element_type in range(1,7):
        if q_by_type[element_type-1][-1] != 0:
            # print(element_type)
            new_entry = [timestep]+q_by_type[element_type-1][1:]
            if len(q_t[element_type-1])==0:
                q_t[element_type-1] = [new_entry]
            else:
                q_t[element_type-1] = q_t[element_type-1]+[new_entry]
